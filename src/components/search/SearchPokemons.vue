	...
	<section :data-search="pokemonSearch.length ? 'on' : ''" class="col-12">
		<form onsubmit="return false" class="col-12 d-flex justify-content-center">
			<div class="col-9 d-flex justify-content-end">
				<input type="text" class="search col-4" v-model="pokemonSearch" />
			</div>
		...
		<div
			...
			<div
				...
				class="col-2 d-flex flex-column align-items-center justify-content-around">
				<img
		...
		computed: {
			filterPokemon() {
				return this.store.state.searchResponse.filter((pokemon) =>
					pokemon.data.name.includes(this.pokemonSearch)
				)
			},
		},
	}
